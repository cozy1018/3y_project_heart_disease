...
# Function to train and evaluate the SVM model with grid search and permutation importance
def train_and_evaluate_svm_with_gridsearch(X, y, cv=5):
    ...
    while len(remaining_features) > 0:
        ...
        # Store all features at this step
    ...
    # Return results including all features used
    return num_features, accuracies, features_used, test_accuracies, all_features_used


# Without SMOTE (as requested)
num_features_no_smote, accuracies_no_smote, features_used_no_smote, test_accuracies_no_smote, all_features_no_smote = train_and_evaluate_svm_with_gridsearch(
    X, y
)
...
plt.title('Test Accuracy vs Number of Features (SVM with GridSearchCV, no SMOTE)')
plt.grid()
plt.show()
